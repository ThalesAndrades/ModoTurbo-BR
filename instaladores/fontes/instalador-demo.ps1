...
#  ModoTurbo (software base sob licenca MIT).
#  a manutencao deste aviso de autoria e obrigatoria.
# ------------------------------------------------------------
# ============================================================
#  MODOTURBO - INSTALADOR COMPLETO (fluxo unico)
...
$installDir = Join-Path $env:USERPROFILE 'ModoTurbo'
$zipUrl = 'https://codeload.github.com/ThalesAndrades/ModoTurbo-BR/zip/refs/heads/main'
...
try {
  ...
  Write-Host '     MODOTURBO - VERSAO DEMONSTRACAO (DEMO)' -ForegroundColor Green
  ...
  if ($cfgText.Contains('pexels_api_keys = []')) {
    ...
    $pexels = [Microsoft.VisualBasic.Interaction]::InputBox(
      ...
      'ModoTurbo DEMO - Chave do Pexels', '')
    if ($pexels.Trim()) {
  ...
  $msg = @(
    '==========================================================',
    '  MODOTURBO - VERSAO DEMO instalada com sucesso!',
    '==========================================================',
    '',
    'O que a DEMO inclui:',
...
$script:installDir = Join-Path $env:USERPROFILE 'ModoTurbo'
$python = Join-Path $script:installDir '.venv\Scripts\python.exe'
$pidFile = Join-Path $script:installDir '.app_server.pid'

function Show-Error([string]$msg) {
  [System.Windows.Forms.MessageBox]::Show($msg, 'ModoTurbo', 'OK', 'Error') | Out-Null
}
...
if (-not (Test-Path $python)) {
  Show-Error ("O ModoTurbo nao foi encontrado em " + $script:installDir + ".`n`nExecute primeiro o Instalar-ModoTurbo-DEMO.bat")
  exit 1
}
...
$lbl.Text = 'Iniciando o ModoTurbo...'
...
$script:tray.Text = 'ModoTurbo - botao direito para opcoes'
$script:tray.Visible = $true

$menu = New-Object System.Windows.Forms.ContextMenuStrip
$miOpen   = $menu.Items.Add('Abrir o ModoTurbo')
$miVideos = $menu.Items.Add('Pasta dos videos prontos')
[void]$menu.Items.Add('-')
$miExit   = $menu.Items.Add('Encerrar o ModoTurbo')
...
$script:tray.ShowBalloonTip(5000, 'ModoTurbo esta rodando',
...
'@
  Set-Content -Path (Join-Path $installDir 'ModoTurboApp.ps1') -Value $appPs1 -Encoding ASCII
...
'@
  Set-Content -Path (Join-Path $installDir 'ModoTurboApp.vbs') -Value $appVbs -Encoding ASCII
  ...
  $lnk = $ws.CreateShortcut((Join-Path $desktop 'ModoTurbo.lnk'))
  $lnk.TargetPath = "$env:WINDIR\System32\wscript.exe"
  $lnk.Arguments = '"' + (Join-Path $installDir 'ModoTurboApp.vbs') + '"'
  $lnk.WorkingDirectory = $installDir
  $lnk.IconLocation = (Join-Path $installDir 'mpt.ico')
  $lnk.Description = 'ModoTurbo - Gerador de videos com IA'
  $lnk.Save()
  $oldBat = Join-Path $desktop 'ModoTurbo.bat'
  if (Test-Path $oldBat) { Remove-Item $oldBat -Force }
  Ok 'Icone verde "ModoTurbo" criado na Area de Trabalho.'

  # ============ PASSO 6: ABRIR O APLICATIVO ============
  Step 6 'Abrindo o ModoTurbo'
  Info 'Uma tela de carregamento vai aparecer e a interface'
  Info 'abre sozinha em uma janela propria. Ate ja!'
  Start-Process wscript.exe ('"' + (Join-Path $installDir 'ModoTurboApp.vbs') + '"')
  ...
  Write-Host '     "ModoTurbo" da sua Area de Trabalho' -ForegroundColor White
  Write-Host ''
