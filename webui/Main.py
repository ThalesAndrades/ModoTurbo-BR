...
with title_col:
    st.title(f"ModoTurbo v{config.project_version}")
                ...
                with llm_helper:
                    ...
                    tips = f"""
                            ...
                            - **Base Url**: 一般为 http://localhost:11434/v1
                                - 如果 `ModoTurbo` 和 `Ollama` **不在同一台机器上**，需要填写 `Ollama` 机器的IP地址
                                - 如果 `ModoTurbo` 是 `Docker` 部署，建议填写 `http://host.docker.internal:11434/v1`{docker_hint}
