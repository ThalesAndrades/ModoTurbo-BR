...
# 设置/ModoTurbo目录权限为777
RUN chmod 777 /ModoTurbo

ENV PYTHONPATH="/ModoTurbo"
...
# docker build -t modoturbo .

# 2. Run the Docker container using the following command
## For Linux or MacOS:
# docker run -v $(pwd)/config.toml:/ModoTurbo/config.toml -v $(pwd)/storage:/ModoTurbo/storage -p 127.0.0.1:8501:8501 modoturbo
## For Windows:
# docker run -v ${PWD}/config.toml:/ModoTurbo/config.toml -v ${PWD}/storage:/ModoTurbo/storage -p 127.0.0.1:8501:8501 modoturbo
